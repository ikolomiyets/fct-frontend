    ...
    <div class="overlay" v-if="showOverlayFlag">
      <div class="overlay-content">
        ...
        <div class="overlay-buttons">
          <button class="accept-button" @click="acceptProject">Accept</button>
          <button class="deny-button" @click="denyProject">Deny</button>
...
export default {
  ...
  methods: {
    async fetchItems() {
      ...
      try {
        const url = `${config.config.backend_url}/getApplications`;
    ...
    async acceptProjectApplication() {
      const config = useConfigStore();
      console.log("testdata: ", this.selectedProject.name, " and ", this.selectedProject.student_id)
      await axios.post(`${config.config.backend_url}/createProject`, {
            id: this.selectedProject.id,
            student_id: this.selectedProject.name,
            student_name: this.selectedProject.student_id
      }, { 
        headers: {
         authorization: `Bearer ${config.accessToken}`,
        }
    ...
    async declineProjectApplication() {
      const config = useConfigStore();
      await axios.patch(`${config.config.backend_url}/declineApplication`, {
            id: this.selectedProject.id,
      }, { 
        headers: {
         authorization: `Bearer ${config.accessToken}`,
